...
func runSetTool(params: CallTool.Parameters) async throws -> CallTool.Result {
  guard let key = params.arguments?["key"]?.stringValue else {
    throw MCPError.invalidParams("Missing required parameter: key")
  }
  guard let value = params.arguments?["value"]?.stringValue else {
    throw MCPError.invalidParams("Missing required parameter: value")
  }
  ...
  for path in paths {
    do {
      ...
      results.append("✗ Error updating \(path): \(error.localizedDescription)")
    }
  }
